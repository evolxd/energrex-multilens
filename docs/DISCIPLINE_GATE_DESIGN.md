...
| 论点纪律 | `mispricing_monitor.thesis_state_for_ticker` (SEVERE/WATCH) | 持仓论点转为 SEVERE 或 WATCH | 3个交易日（给复核论点的时间，不强制当天砍仓） |

窗口数字是本次给出的默认建议，不是拍死的——跟 K=20、Wilson 95% 一样，先定一版能跑起来的，
用真实数据观察一段时间后再调。
...
1. **论点纪律要不要先做**——它依赖的 `thesis_state_for_ticker` 已经存在且在跑，
   接入成本不高，但"响应"的定义比其它四个模糊（SEVERE 状态下"减仓"和"清仓"都算
   合理响应，不是非黑即白的"平不平仓"）。倾向先做另外四个（止盈/止损/到期/对冲，
   都是清晰的二元判断），论点纪律放第二批。
2. **响应窗口数字**（2个交易日/1个交易日/3个交易日）目前是我给的默认建议，没有
   历史数据支撑，需要跑一段时间实际观察后再调整，不是这次就要精确定死。
3. **未处理信号要不要主动推送提醒**（而不是只在门⑤页面被动展示）——这个依赖已经
   搭好的手动同步流程，倾向先不加主动推送，等看了几周的数据分布再决定要不要加。

---

## 8. 变更记录

- **2026-09-08**：首次成稿。动机、维度定义、"acted vs self_resolved"核心判断逻辑、
  数据模型、记录算法、分维度打分方案全部定下来，代码还未开始写。
