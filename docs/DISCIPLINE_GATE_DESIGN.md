...
## 9. 推送机制——三条可行路径，需要用户选一条

"推送"在这个系统的实际环境下有三种不同量级的实现方式，选哪条决定了要不要新建
独立于 Streamlit 之外的运行进程：
...
**关键区别在"要不要脱离你打开 app 这个动作独立触发"**——方案 A 仍然是"你自己点了同步，
顺便告诉你"，跟这周投资者看板那条线定下的"每次你手动同步后我顺手推一次"是同一个模式；
方案 B/C 才是真正意义上"不管你开不开app，到点都会通知你"的推送。

**需要用户确认**：是要 A（跟手动同步绑在一起，最省事）还是 B/C（真正独立于你打开
app 这个动作的定时推送）？如果是 B/C，还需要再确认 C 里"Claude Code 定时任务"这种
形式你是否能接受（意味着提醒渠道依赖这个对话环境，而不是纯粹这个 repo 自己的代码）。
...
- **2026-09-08**：首次成稿，随后同一天用户确认：论点纪律这轮不做、响应窗口数字
  照定不改、需要主动推送提醒（不只是被动展示）。新增 §9 记录推送机制的三条路径，
  等用户选定后再动代码。
