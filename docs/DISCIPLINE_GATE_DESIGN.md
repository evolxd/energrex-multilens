...
> **这份是 v1（已建，2026-09-09）：4 个维度（止盈/止损/到期/对冲）、分维度
> 响应率、不合成总分。** 2026-09-10 一次 grilling 会话把范围大幅扩到"全部
> 风控信号的响应考核 + 门④事后检测 + 百分制纪律分"——完整架构见
> [`DISCIPLINE_AND_REVIEW_ARCHITECTURE.md`](DISCIPLINE_AND_REVIEW_ARCHITECTURE.md)。
> 这份的表结构（`discipline_signals`）和核心判断（acted vs self_resolved）v2
> 继续沿用，不推翻；但"止盈单独算一个纪律维度"这条 v2 要拿掉（提前止盈是
> 选择不是义务）。
