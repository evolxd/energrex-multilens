...
## 6. 已知遗留 / 待办（本轮不处理，留给独立任务）

**本地服务器日期 vs 美东（ET）午夜时区漂移**：`compute_portfolio_stress_test`
的 `dte = max(0, (exp_date - today).days)` 依赖 `today`，现状里 `today` 未传时
退回裸 `datetime.date.today()`——取的是**进程所在服务器的本地时区日期**（开发机
上实测是 Pacific），不是基于账户体系里 ET 感知的 `now` 派生的日期。这意味着
在 ET 午夜前后（服务器为 Pacific 时区时，大约是太平洋时间晚 9 点到午夜之间），
"服务器本地日期"和"ET 日历日期"会相差一天，从而让同一时刻算出不同的 `dte`，
进而影响 Greeks/冲击定价分支是否触发。

已用 `freezegun` 实测验证：冻结到 `2026-09-23T23:30:00-04:00`（ET 晚上 11 点半，
ET 日历日期仍是 9-23）时，`datetime.date.today()` 在本机环境下返回的是
`2026-09-24`——比 ET 日历日期多了一天。这是**现状代码本来就有的潜在口径问题**，
不是本次架构搬家引入的。

**用户裁决（2026-09-23）**：阶段三严格遵守"行为保真"——`today` 作为可选注入
参数落地时，默认值继续维持现状的 `datetime.date.today()`（服务器本地日期），
**不在这次架构搬家里隐式修正或变更时区口径**。是否要把 `today` 的默认来源
改成 ET 感知的 `inputs["now"].date()`（从而修正这个潜在的午夜漂移），留给
未来一个独立的、有自己特征测试保护的任务去决定和实施。
