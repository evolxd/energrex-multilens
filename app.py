...
with st.sidebar:
    ...
    page = st.radio(
        ...
        options=["🏆 排行榜", "🔍 单股详情", "🏗️ 结构分析", "⚖️ 对比分析",
                 "🔬 评分审计", "📝 数据编辑"],
...
# 页面 2.5：结构分析（估值之后的一步）
#
# 六维评分把一家公司压成一个点：revenue_growth_yoy = 0.85 对一家四条腿
# 各涨 21% 的公司和一家全靠一个分部的公司是同一个数。这一页展开那个点，
# 回答"这个分数是靠几条腿撑起来的"。
#
# 定量部分来自 scoring/company_structure.py（SEC XBRL 分部收入）。定性
# 部分是 Fisher 十五要点，走 .claude/skills/fisher-company-analysis——
# 那部分刻意不在这里自动打分，理由见该 skill 的 §4：给不可观测的东西编
# 一个分数，正是 get_category() 当年把 COST 默认成 AI_SOFTWARE 犯过的错。
# ══════════════════════════════════════════════════════════
elif page == "🏗️ 结构分析":
    ...
    _sel_struct = st.session_state.get("selected_ticker", TICKERS[0])
    if _sel_struct in TICKERS and st.session_state.get("struct_sel") != _sel_struct:
        st.session_state["struct_sel"] = _sel_struct
    struct_ticker = st.selectbox(
        "选择股票", df["ticker"].tolist(), key="struct_sel",
        on_change=_sync_selected_ticker, args=("struct_sel",),
    )

    from scoring.company_structure import assess as _assess_structure
    from scoring.exposure_context import non_scored_chain as _non_scored_chain
