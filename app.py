...
# ══════════════════════════════════════════════════════════
    ...
    if row["valuation_score"] >= 65:
        val_text = f"**估值合理** — PEG {peg:.2f}x，在同类中属于便宜区间，当前价格有一定安全边际。"
    elif row["valuation_score"] >= 45:
        val_text = f"**估值偏贵** — PEG {peg:.2f}x，需要成长持续兑现才能支撑当前倍数。"
    else:
        val_text = f"**估值极高** — PEG {peg:.2f}x，远超合理区间，任何成长放缓都可能引发大幅回调。"

    # 成长判断
    if row["growth_score"] >= 75:
        grow_text = f"**成长强劲** — 收入增长 {rev_g:.0%}，超出行业平均水平，成长动能充足。"
    elif row["growth_score"] >= 50:
        grow_text = f"**成长稳健** — 收入增长 {rev_g:.0%}，符合预期，无明显加速迹象。"
    else:
        grow_text = f"**成长趋缓** — 收入增长 {rev_g:.0%}，增速低于同类优质公司水平。"

    # 质量判断
    if r40 >= 60:
        qual_text = f"**质量出色** — Rule of 40 = {r40:.0f}（FCF口径），属于同类顶级水平。"
    elif r40 >= 40:
        qual_text = f"**质量合格** — Rule of 40 = {r40:.0f}，超过40基准线，盈利能力基本达标。"
    else:
        qual_text = f"**质量待改善** — Rule of 40 = {r40:.0f}，低于40基准，增长效率有待提升。"

    # AI暴露（用评分代理）
    if ai_score_val >= 75:
        ai_text = f"**AI核心资产** — AI暴露评分 {ai_score_val:.0f}，是真正的AI受益者，而非边缘参与者。"
    elif ai_score_val >= 50:
        ai_text = f"**AI中等暴露** — AI暴露评分 {ai_score_val:.0f}，受益于AI浪潮但非核心标的。"
    else:
        ai_text = f"**AI暴露有限** — AI暴露评分 {ai_score_val:.0f}，AI叙事支撑力度不足。"

    # 风险
    if row["risk_penalty"] >= 10:
        risk_text = f"**高风险** — Beta {beta:.2f}，风险扣分 {row['risk_penalty']:.1f}，波动率高，需控制仓位。"
    elif row["risk_penalty"] >= 6:
        risk_text = f"**中等风险** — Beta {beta:.2f}，风险扣分 {row['risk_penalty']:.1f}，正常成长股风险水平。"
    else:
        risk_text = f"**低风险** — Beta {beta:.2f}，风险扣分仅 {row['risk_penalty']:.1f}，防御性相对较强。"
    ...
    # ── 多维智库分析 Prompt ────────────────────────────────────
    st.markdown("#### 🧠 多维智库分析 Prompt")
    st.caption("将以下 Prompt 直接发给 Claude，获取 8 位顶级投资人框架的交叉验证。")

    _v_s  = int(row["valuation_score"])
    _g_s  = int(row["growth_score"])
    _q_s  = int(row["quality_score"])
    _ai_s = int(row.get("ai_exposure_score") or 0)
    _eg_s = int(row.get("expectation_gap_score") or 0)
    _rs   = float(row["raw_score"])
    _rp   = float(row["risk_penalty"])
    _fs   = float(row["final_score"])
    _co   = row.get("company", ticker)
    _ct   = cat.value

    _w = WEIGHT_CONFIG[cat]
    _wv, _wg, _wq, _wa, _we = (
        f"{_w.valuation:.0%}", f"{_w.growth:.0%}", f"{_w.quality:.0%}",
        f"{_w.ai_exposure:.0%}", f"{_w.expectation_gap:.0%}"
    )

    _prompt_full = f"""你是一个整合了多位顶级投资人思维框架的分析系统。
我需要你对以下股票的评分维度做深度的智库交叉验证。
...
### 📐 维度1：估值（Damodaran 估值纪律 —— 用大白话讲透）
这一步要回答一个最根本的问题：现在这个价格，买的到底是"公司值多少钱"（价值），
还是"大家愿意为它的故事付多少钱"（定价）？这两个东西经常被混为一谈——一个热门赛道的股票
涨得再猛，也不代表它真的更值钱，可能只是大家愿意为同一个故事出更高的价而已。

具体要检查：
1. 先把公司未来能赚的现金流折现算出一个"内在价值"，而不是直接看别人愿意出多少钱买。
2. 用现在的股价倒推出"市场认为这家公司未来能长多快、赚多少钱"——这个隐含假设离不离谱？
   增长速度是不是比历史上任何一家同类公司都快？利润率是不是已经假设到了行业最优水平？
3. 检查"终值"（很多年以后这家公司还值多少钱）占整个估值的比例。如果超过 80%，
   说明你几乎是在为一个遥远、不确定的未来买单，而不是为看得见的近几年现金流买单——
   这种股票的估值特别容易被情绪推高推低。
4. 再看 ROIC（投入资本回报率）能不能撑得住这个增长假设——如果一家公司要维持高增长，
   通常得不断往里砸钱扩张，如果它的资本回报率本来就不高，砸钱越多反而越不划算，
   这时候"增长"不是好事，是烧钱。
5. 最后回过头看：现在给的估值分 {_v_s} 分，是不是跟这套 DCF 内在价值逻辑吻合？
   如果分数很高但上面几条越看越不对劲，说明分数可能是被"故事"带偏了，不是被"数字"撑起来的。

### 🚀 维度2：成长（Cathie Wood + Jensen Huang —— 用大白话讲透）
这一步要判断：这家公司的成长，是真的处在一条陡峭向上的曲线上，还是已经开始放缓、
只是还没被市场发现？

**Cathie Wood 角度**：
- 每个颠覆性技术都会经历一条 S 型曲线——一开始很慢（早期采用者阶段），然后突然加速
  （渗透率跨过临界点后所有人都要用），最后见顶放缓（普及到头了）。要判断这家公司现在
  具体在哪个阶段：如果还在早期，意味着未来还有好几倍的渗透空间；如果已经在陡峭爬升的
  中段，意味着增长最快的日子可能已经过去一部分了；如果开始进入平台期，那用现在的高增长
  去线性外推未来，大概率会被打脸。
- 这个赛道的"成本曲线"（Wright's Law：产量每翻倍一次，成本按固定比例下降）是不是还在
  按历史规律持续下降？如果下降速度比预期快，意味着技术会比市场想的更快渗透到更多场景；
  如果成本下降停滞，增长故事可能会比预期更早遇到天花板。
- 五年后这个市场真实能有多大？不是公司 PPT 里画的 TAM，而是刨除重复计算、过度乐观假设
  之后，真正能兑现的市场规模。

**Jensen Huang 角度**：
- 在 AI 产业这个大分工体系里，这家公司到底是卖铲子的（基础设施/芯片），还是搭平台的，
  还是做最上层应用的？不同层级的护城河、竞争烈度、议价能力完全不一样。
- 它是不是在一个"现在还只是零到十亿美元规模，但未来会变成千亿美元"的早期赛道里占了
  位置？越早占住关键卡位，后面享受到的成长红利越大。
- 全栈视角：它到底控制了产业链里的哪几层？如果只控制一层，瓶颈（最赚钱、最有话语权的
  环节）会不会随着技术演进转移到别的层去，把它甩在后面？

综合看：现在给的成长分 {_g_s} 分，到底是被"处在陡峭 S 曲线上升期 + 卡住了关键位置"这种
结构性优势撑起来的，还是仅仅因为最近几个季度数字好看？

### 🛡️ 维度3：质量（Howard Marks —— 用大白话讲透）
这一步不是看"这家公司好不好"，而是看"现在这个时候，它的好在多大程度上已经被定价了，
以及万一判断错了，会亏到什么程度"。

- 市场情绪的钟摆：现在整个板块/这只股票的情绪，是处在"人人都爱、极度乐观"的贪婪端，
  还是"没人理会、极度悲观"的恐惧端，还是中间地带？钟摆最终都会摆回中间——如果现在是
  贪婪端，即使公司质量真的很好，买入的时点也可能不划算，因为未来几年的好消息已经被
  提前花掉了。
- 永久性资本损失 ≠ 短期波动。真正要担心的风险，不是股价明天跌 20%（这只是波动，会回来），
  而是这家公司的护城河/商业模式本身出了根本问题，导致钱真的回不来了。要分清楚：现在看到
  的"质量"风险信号，是短期噪音还是真的动摇了根基？
- 护城河到底是结构性的（网络效应、转换成本高、专利/技术壁垒），还是周期性的红利（正好
  赶上行业景气周期、涨价潮，一旦周期反转优势就消失）？结构性护城河能穿越周期，周期性
  红利不能。
- 综合这几条，现在的周期温度（偏热还是偏冷）是不是应该让质量分 {_q_s} 打一个折扣，
  或者反而应该加分？

### 🤖 维度4：AI暴露（Jensen Huang + Satya Nadella —— 用大白话讲透）
这一步要搞清楚：这家公司说自己"吃到 AI 红利"，到底是真的靠 AI 赚到了增量的钱，
还是只是蹭上了 AI 这个热词。

**Jensen Huang 角度**：
- 在 AI 从训练模型、到部署推理、再到变成实际应用这条链条里，这家公司具体站在哪一环？
  越靠近底层基础设施（芯片、算力）的公司，短期最先受益但也最容易被替代；越靠近上层
  应用的公司，护城河取决于是否有独特数据和用户粘性。
- 它是纯粹的"卖水人"（基础设施层），还是搭了一个别人离不开的平台，还是做的是最终
  用户直接用的产品（应用层）？这三种角色能吃到的 AI 红利大小和持续时间完全不同。

**Satya Nadella 角度**：
- 这家公司怎么把 AI 变成真金白银？是像"卖算力/调用次数"这种 Token 工厂模式（本质是
  卖资源，议价权弱、容易被价格战卷进去），还是像 App Server 那样把 AI 包装成企业离不开
  的整体解决方案（议价权强，客户粘性高）？
- 有没有别人拿不到的数据护城河？如果有，它就能对生态系统里的其他玩家收"过路费"；
  如果没有，它做的 AI 功能迟早会被更大的平台方直接抄过去、内嵌进自己的产品里，白忙一场。
- 综合看，现在给的 AI 暴露分 {_ai_s} 分，是靠上面这些结构性优势撑起来的，还是这几个季度
  财报电话会上多提了几次"AI"这个词，被市场用叙事定价撑起来的？

### 📊 维度5：预期差（Howard Marks + Mary Meeker —— 用大白话讲透）
这一步要回答：现在这个股价里，"好消息"已经被提前计价了多少，市场还有没有被这家公司
"吓到"或者"看走眼"的空间。

**Howard Marks 角度**：
- 二阶思维：不要只想"这家公司好不好"（一阶），要想"大家是不是已经知道它好，并且已经把
  这个好消息体现在股价里了"（二阶）。如果全世界都已经认定它是好公司，那么即使它真的
  继续好下去，股价也未必再涨，因为好消息已经花完了。
- 现在的情绪到底是"人人看好、生怕错过"，还是"没人关注、被市场忽略"？后者才是真正的
  机会所在——因为预期低，随便一个不错的季度就能造成"超预期"的惊喜；前者反而危险，
  因为预期已经很高，稍有不及预期就会被市场狠狠教训。

**Mary Meeker 角度**：
- 大家看到的"渗透率还在快速提升"这个事实，是不是已经被写进了分析师模型、写进了股价？
  如果所有人都已经在用同一份增长曲线做外推，那这个曲线本身已经不能带来超额收益了。
- 有没有一些全球视角的、还没被主流市场注意到的信号（比如某个新兴市场率先验证了商业
  模式，或者海外某国的渗透率数据领先其他地区几个季度）？这些领先指标经常能提前预示
  接下来会发生的事，但大部分本土投资者压根没看。
- 综合看，现在给的预期差分 {_eg_s} 分，意味着剩下多大的"超预期空间"——是几乎被定价
  定满了，还是还有相当大一块还没被市场认识到？

---

## 元判断（Peter Thiel 视角）
不参与子维度打分，只做整体逻辑检验：
- 这家公司的核心业务是 0→1（创造新类别）还是 1→N（复制扩张）？
- 它的护城河秘密是什么——大多数人不相信但正确的真相？
- 10 年后，它是这个市场的"最后赢家"吗？
...
请最终给出：
1. **维度共振信号**：哪些维度被多个框架共同强化？
2. **最大分歧点**：哪个维度的评分与智库判断最不一致？
3. **风险扣分是否充分**：当前 {_rp:.2f} 的风险扣分，Marks 视角下是否低估了？
4. **一句话结论**：如果这 8 位投资人坐在一起，他们会在哪里形成共识，在哪里激烈争论？"""

    _prompt_quick = f"""用以下智库框架快速验证 {ticker} 的评分：

估值{_v_s} · 成长{_g_s} · 质量{_q_s} · AI暴露{_ai_s} · 预期差{_eg_s}
综合{_rs:.1f}/100，风险扣分{_rp:.2f}，Final Score {_fs:.1f}

- 估值：Damodaran——价值还是定价？增长假设合理吗？
- 成长：Cathie Wood——S曲线哪个阶段？Jensen——平台角色？
- 质量：Howard Marks——钟摆在哪？永久损失风险？
- AI暴露：Jensen+Nadella——基础设施还是应用层？变现路径？
- 预期差：Marks——共识定价了多少？Meeker——数据佐证？
- 元判断：Thiel——0→1还是1→N？最后赢家吗？

指出最大的分歧点和共识点，100字以内。"""

    _pv = st.radio(
        "Prompt 版本",
        ["📋 完整版（深度研究 / 买入决策）", "⚡ 快速版（日常复盘 / 3分钟）"],
        horizontal=True, key="prompt_ver",
        label_visibility="collapsed"
    )
    _prompt_text = _prompt_full if "完整" in _pv else _prompt_quick
    st.text_area(
        "复制以下 Prompt 发给 Claude：",
        value=_prompt_text,
        height=560 if "完整" in _pv else 180,
        key="prompt_output"
    )
