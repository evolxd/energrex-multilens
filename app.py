...
### 📐 维度1：估值（Damodaran 视角）
- 当前定价反映的是"价值"还是"定价"？两者是否出现混淆？
- 市场隐含的增长假设是什么？ROIC 和再投资率能否支撑？
- 终值占总估值比例是否过高（>80% 需警惕）？
- 当前估值分 {_v_s} 是否与 DCF 内在价值框架一致？

### 🚀 维度2：成长（Cathie Wood + Jensen Huang 视角）
**Cathie Wood**：
- 该公司处于哪条技术 S 曲线的哪个阶段？
- 成本曲线（Wright's Law）的方向和斜率如何？
- 真实 TAM 在 5 年后有多大？

**Jensen Huang**：
- 该公司在 AI 产业平台层级中扮演什么角色？
- 是否处于零十亿美元市场的早期布局位置？
- 全栈视角：它控制了哪几层？瓶颈会迁移到哪层？

- 成长分 {_g_s} 是否被 S 曲线阶段和平台角色支撑？

### 🛡️ 维度3：质量（Howard Marks 视角）
- 钟摆当前在哪里？贪婪极端 / 中间 / 恐惧极端？
- 永久资本损失的真实概率是多少（≠ 波动率）？
- 护城河的持久性：是结构性优势还是周期性红利？
- 当前周期温度是否需要对质量分 {_q_s} 打折？

### 🤖 维度4：AI暴露（Jensen Huang + Satya Nadella 视角）
**Jensen Huang**：
- 该公司在 AI 工厂（训练/推理/应用）中的具体位置？
- 是基础设施层、平台层还是应用层？

**Satya Nadella**：
- AI 变现路径：Token Factory 模式还是 App Server 模式？
- 数据护城河是否存在？能否收"生态系统税"？
- AI 暴露分 {_ai_s} 是结构性的还是叙事驱动的？

### 📊 维度5：预期差（Howard Marks + Mary Meeker 视角）
**Howard Marks**：
- 二阶思维：共识是什么？共识已定价了多少乐观？
- 市场情绪处于哪个阶段——人人看好 or 被忽视？

**Mary Meeker**：
- 采用曲线的速度是否已被充分定价？
- 是否有全球视角的数据被市场忽视（如新兴市场信号）？
- 预期差分 {_eg_s} 意味着剩余超预期空间有多大？
...
指出最大的分歧点和共识点，100字以内。"""
    ...
    st.text_area(
        ...
        value=_prompt_text,
        height=320 if "完整" in _pv else 180,
        key="prompt_output"
    )
    st.caption(
        f"💡 配合 Damodaran / Cathie Wood / Howard Marks / Jensen Huang / "
        f"Satya Nadella / Mary Meeker / Peter Thiel / Charlie Munger Skill 使用效果最佳。"
    )
