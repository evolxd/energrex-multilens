...
from account.spread_pairing import build_spread_portfolios as _build_spread_portfolios_pure
...
_BETA_BASE = {
    ...
    # 2026-09-14：yfinance .info["beta"] 对 SMH 返回 None（很多ETF在Yahoo
    # 都是这样，不是数据缺失，是Yahoo没给这个字段），_refresh_beta_spy()
    # 也只会去刷新已经在这本字典里的标的，SMH此前压根没被尝试过。这里手动
    # 按同一套方法（5年月度收益率对SPY回归）算了一次：5年=1.77、3年=2.03、
    # 1年=2.63——beta在明显走高，跟这几年AI芯片周期里SMH波动率相对大盘持续
    # 放大是一致的。取5年期作为基准值，跟其余标的的方法保持一致；未来做
    # SMH对冲择时，1年期这个更高的数字更能反映"现在"的敏感度，可以在
...
# Module B: 交易绩效分析
#
# 2026-09-09：_wilson_interval / _compute_performance_stats 搬进了
# account/performance.py（真正可 import 的模块，不再是这个UI脚本里的
# 模块级函数）——门③仓位管理页面现在也要用这里的 win_rate_shrunk/
# kelly_f_shrunk 数据（用户拆分决定：绩效留门⑥，仓位建议去门③），
# 不能再只靠 _cascade.py::_get_am() 那个只有它自己在用的 AST 技巧。
...
# ─────────────────────────────────────────────────────────────────
# 2026-09: the pure pairing/recognition logic moved to account.spread_pairing
# (no streamlit import, no I/O -- see docs/architecture/account_monitor.md and
# docs/REFACTORING_WORKFLOW.md Phase 3). This is now a thin wrapper: the only
# thing it still does is the DB read, then it hands the DataFrame off.
...
def _build_spread_portfolios(acct_id: str) -> list[dict]:
    ...
    """
    df = _load_options_positions(acct_id)
    return _build_spread_portfolios_pure(df, datetime.date.today())
...
with _pos_tabs[1]:
    ...
    if _stats is None:
        ...
        # ── 组合策略分析 ──
        # 2026-09-09：这张表只留纯历史事实（胜率/均赢均亏/赔率b/原始Kelly），
        # 收缩胜率/收缩Kelly/95%置信区间这几个"仓位建议"层的数字搬去了
        # 门③仓位管理页面——用户拆分决定：绩效留门⑥，仓位建议去门③，
        # 两处看的是同一批底层数据（account/performance.py），不是两份口径。
        st.markdown("**按组合策略分组**")
        if _stats.get("by_combo"):
            st.caption(
                "赔率 b = 均赢/|均亏|；原始 Kelly f* = 胜率 − (1−胜率)/b，未打小样本折扣、"
                "未过硬约束封顶，纯历史事实。带收缩估计/置信区间的仓位建议版本见 "
                "⚖️ 仓位管理（门③）。"
            )
